 ...
 * role="status"/"alert" already makes it a live region. That's a
 * deliberate opt-out, not a gap — routing Toast through this shared
 * region too would double-announce every toast. Don't "fix" Toast to use
 * this without removing its own role="status"/"alert" first.
 */
