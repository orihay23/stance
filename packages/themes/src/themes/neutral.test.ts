...
describe("neutral theme", () => {
  const output = compileTheme(neutral);

  it("compiles without throwing and scopes both blocks", () => {
    expect(output).toContain('[data-theme="neutral"] {');
    expect(output).toContain('[data-theme="neutral"].dark {');
  });
  ...
  it("defines light and dark values differently for every color role", () => {
    const lightBlock = output.slice(0, output.indexOf('[data-theme="neutral"].dark {'));
    const darkBlock = output.slice(output.indexOf('[data-theme="neutral"].dark {'));
