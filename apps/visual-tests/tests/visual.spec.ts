...
// Only the "light"/"dark" captures from lightDarkCaptures() are the
// theme-comparison matrix the Definition of Done means by "3 themes ×
// light/dark = 6 combinations" — every story's Light + Dark variant reads
// its data-theme from the shared `storyTheme` ref (apps/playground/src/
// useStoryTheme.ts), itself seeded from this same `?theme=` query param, so
// looping themes here needs no per-story wiring. Every other capture
// (narrow-container responsive checks, DataTable's extra pagination/
// selection/filtering demo variants, etc.) stays single-theme — multiplying
// those by 3 wouldn't test anything the light/dark matrix doesn't already
// cover, and would triple the screenshot count for no benefit.
const THEMED_CAPTURE_NAMES = new Set(["light", "dark"]);
    ...
    for (const variantSpec of componentSpec.variants) {
      test.describe(variantSpec.variantTitle, () => {
        for (const capture of variantSpec.captures) {
          const themesForCapture = THEMED_CAPTURE_NAMES.has(capture.name) ? allThemes : [allThemes[0]!];
